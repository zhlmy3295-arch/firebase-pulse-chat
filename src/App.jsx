...
import { User as UserIcon, Phone, MessageSquare, UserPlus } from "lucide-react";
import { db, ref, update, onValue, get } from "./lib/firebase";
...
export default function App() {
  ...
  const [targetPhones, setTargetPhones] = useState(null);
          ...
          <form onSubmit={handleJoin} className="space-y-4" dir="rtl">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  className="block w-full pl-10 pr-3 py-3 border-slate-800 rounded-xl bg-slate-900/50 text-white focus:ring-2 focus:ring-indigo-500 outline-none"
            ...
            {isSignUpMode && (
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input type="number" className="w-full px-4 py-3 border-slate-800 rounded-xl bg-slate-900/50 text-white outline-none" value={age} onChange={(e) => setAge(e.target.value)} min="10" max="100" required />
                </div>
                <div className="flex-1">
                  <label className="block text-sm font-medium text-slate-300 mb-1.5">النوع</label>
                  <select className="w-full px-4 py-3 border-slate-800 rounded-xl bg-slate-900/50 text-white outline-none" value={gender} onChange={(e) => setGender(e.target.value)}>
